...
const CompressPdf = () => {
  ...
  const handleFileChange=(e) =>{
    ...
    if (fileName) {
      ...
      if(extension == 'pdf'){
        ...
        setSelectedFile("");
        setDownloadError("Error Occured: You have not selected PDF file");
      }
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:3000/api/compress_pdf', formData, {
        responseType: 'json',
      });
      ...
      if (filePath && filePath.startsWith('/')) {
        ...
        setDownloadError('');
        setConvert('File Compress Successfully');
      } else {
        ...
        <div className="container">
            <div className="flex-center">
                <div className="border-box">
                    <h1 className="title">Compress PDF</h1>
                    <p className="description">Compress Your Pdf documents with incredible accuracy.</p>
                    
                    <div className="file-input-container">
                        <input type="file" className="hidden" id="FileInput" onChange={handleFileChange}/>
                        <label htmlFor="FileInput" className="file-label">{selectedFile ? selectedFile.name : "Select File"}</label>
                        <button disabled={!selectedFile || loading} className="convert-button btn btn-danger" onClick={handleSubmit}>
                          {loading ? 'Loading...' : 'Compress File'}
                        </button>
